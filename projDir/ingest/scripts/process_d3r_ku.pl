...
$RAW_DIR = "/home/disk/bob/olympex/raw/d3r/ku";
$TMP_DIR = $RAW_DIR."/tmp";
...
for $idate (0..$#dates) {
    ...
    $last_sweep = 0;
    $first_file = 1;
    foreach $file (<KuD3R*.nc>) {
	print "file = $file\n";
	($yyyymmdd,$hhmmss,$sweep) = ($file =~ /KuD3R_(.*)_(.*)_(.*)\.nc/);
	...
	if ($sweep > $last_sweep) {
	    ...
	    $command = "$BIN_DIR/RadxConvert -params $PARAM_DIR/RadxConvert.d3r.ku.archive -f $TMP_DIR/*.nc";
	    print "   command = $command\n";
	    #system($command);
